...
import { Parser, Player, DB, WebGLRenderer, CanvasRenderer } from './index'

const canvas = document.getElementById('canvas') as HTMLCanvasElement

/**
 * 基本使用 - 使用新的 renderers API
 */
...
const TESTCASE1 = async (): Promise<void> => {
  ...
  if (canvas !== null) {
    const player = new Player({
      container: canvas,
      loop: true,
      // 新 API: 传入 renderer 实例，按顺序尝试
      // 如果不传，默认使用 CanvasRenderer
  ...
  player.onEnd = () => {
    ...
    const playMode =
      player.config.playMode === PLAYER_PLAY_MODE.FORWARDS
        ? PLAYER_PLAY_MODE.FALLBACKS
        : PLAYER_PLAY_MODE.FORWARDS
    player.setConfig({
      loop: 1,
      playMode
    })
